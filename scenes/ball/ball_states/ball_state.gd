...
func process_gravity(delta: float, bouncines: float = 0.0) -> void:
	if ball.height > 0 or ball.height_velocity > 0:
		ball.height_velocity -= GRAVITY * delta
		ball.height += ball.height_velocity
		if ball.height < 0:
			ball.height = 0
			if bouncines > 0 and ball.height_velocity < 0:
				ball.height_velocity = -ball.height_velocity * bouncines
				ball.velocity *= bouncines
